...
- **简历上传历史**：自动保留最近 **10** 份原件（`backend/data/resume_uploads/`，不入库 git）；解析失败也会保存，可在底栏「历史」或简历优化页选用、删除。列表按**最近使用在下方**排序（与能力分析「新在上」相反）
- **求职看板（仅 Electron 桌面）**：本地 SQLite 多维表格 + **管道视图**（左侧阶段导航 / 列内拖动手柄排序并写入 `sort_order`、卡片上 **阶段下拉** 快速改列、可勾选 **显示已结束**）；Offer 录入与多 Offer 对比；浏览器/手机端不显示该入口
- **多模型**：OpenAI 兼容 API，界面切换；支持 Think、识图；主模型不可用时自动降级
- **桌面/网络**：Electron 窗口（屏幕共享隐身、Boss Key）或浏览器访问（局域网扫码）
- **设置 / 配置**：顶栏齿轮为常用设置（答案展示卡片/流式、扫码、快捷词、**配色方案**等）；滑块按钮为「配置」（模型并行、VAD、LLM 参数）。答案区可选**卡片**（框内滚动）或**流式**（自上而下通读，多路同时生成时带提示条）
- **配色**：设置中可选 **Dark+ / Light+ / Dark 高对比**（参考 VS Code），本地 `localStorage` 记忆；代码块使用 Prism 对应浅色/深色/高对比样式，**选中文本**按主题单独配色以保证对比度（不随字体变化）

### 多模型并行与来源

- **顶栏下拉**选择「**优先**」模型：实时辅助会**优先**把题目分给该模型（空闲、已启用、健康且满足识图等条件时）。**多题并行**时，其余题目按**配置里模型列表从上到下**的顺序依次占用剩余路数。
- 在**设置 → 配置**里可调整模型**顺序**（上/下移）、**启用/关闭**，并设置**最大并行路数**（`config.json` 中 `max_parallel_answers`）。关闭的模型不参与答题。
