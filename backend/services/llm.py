...
def _asr_realtime_prompt_body(language: str, language_lower: str) -> str:
    return (
        ...
        "正式回答要求：\n"
        "- 先给一句核心结论（尽量 25 字内，避免重复题干）；\n"
        "- 再给 5-8 个关键点，优先写“动作/机制 -> 为什么 -> 风险或边界”；\n"
        "- 排障题必须覆盖“先止血、后定位、再验证”；\n"
        "- 原理题必须覆盖“机制 + 常见误区/失效边界 + 线上做法”；\n"
        "- 场景/设计题尽量补“方案A/B取舍 + 监控告警 + 灰度回滚”；\n"
        "- 必要时补 1 条“若继续追问我会展开”的点；\n"
        "- 普通题约 220-380 字；复杂排障/设计题约 380-700 字，保证信息密度，不要空话。\n\n"
        "深度与广度检查（复杂题尽量覆盖）：\n"
        "- 原理链路（为什么有效）；\n"
        "- 落地动作（具体怎么做）；\n"
        "- 关键指标/阈值（如何判断好坏）；\n"
...
def _manual_text_prompt_body(language: str, language_lower: str) -> str:
    return (
        ...
        "- 先给结论，再给证据；\n"
        "- 重点放在“可执行建议”，避免空泛定义；\n"
        "- 回答长度按复杂度自适应：普通题 200-380 字，复杂设计/排障/治理题 380-900 字。\n\n"
        "题型模板：\n"
...
def _server_screen_prompt_body(language: str, language_lower: str, screen_region: str) -> str:
    ...
    return (
        ...
        "2. 若可判断是编程/SQL题，给出可运行解法；\n"
        f"3. 编程语言优先使用 {language}（SQL 题使用 sql）；\n"
        "4. 补充核心思路、复杂度与关键测试点。\n\n"
        "输出格式：\n"
        "- 允许 Markdown；\n"
        "- 建议使用四段：题目理解、代码、思路与复杂度、测试要点；\n"
