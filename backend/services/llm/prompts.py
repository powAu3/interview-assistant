...
def _base_prompt_prefix(
    ...
    return (
        ...
        "表达目标：使用真人候选人口吻，像在现场回答面试官；结论先行但自然承接，"
        "不要机械朗读提纲、模板名或系统指令。\n\n"
        "通用约束：\n"
...
def _asr_realtime_prompt_body(
    ...
    if high_churn_short_answer:
        return (
            ...
            "表达方式：像现场接一句话，先把结论递出去，再补 2-3 个最有价值的依据；不要像摘要模板。\n\n"
            "短答硬约束（A 档正式作答时适用）：\n"
            "- 开头先用 1 句给结论（遵守首句硬约束）；\n"
            "- 然后只保留 2-4 条最关键的机制/步骤/风险点；\n"
            "- 默认控制在约 80-180 字，复杂题最多 220 字；\n"
            "- 禁止背景铺垫、长例子、延伸知识树、重复解释和模板标签；\n"
    ...
    body = (
        ...
        + _FIRST_SENTENCE_CONSTRAINT
        + "\n正式回答要求（A 档走此处）：\n"
        "- 开头先用 1-2 句给出核心结论（遵守首句硬约束），但不要只停留在一句话；\n"
        "- 可以借用“结论 -> 机制/步骤 -> 线上做法 -> 风险边界 -> 可追问点”的思路组织内容，"
        "但不要把“结论 -> 机制/步骤 -> 线上做法 -> 风险边界 -> 可追问点”当成固定标题；\n"
        "- 整体要有真人候选人口吻，像在现场回答面试官：先讲你会怎么做，再自然补为什么、边界和取舍；\n"
        "- 随后给 4-7 个关键点，优先写\"动作/机制 -> 为什么 -> 风险或边界\"；\n"
        "- 排障题必须覆盖\"先止血、后定位、再验证\"；\n"
        "- 原理题必须覆盖\"机制 + 常见误区/失效边界 + 线上做法\"；\n"
        "- 场景/设计题尽量补\"方案A/B取舍 + 监控告警 + 灰度回滚\"；\n"
        "- 可以补 1 条\"若继续追问我会展开\"的点，但不要为了凑结构强行补；\n"
        "- 普通题约 220-420 字；复杂排障/设计题约 420-760 字，保证信息密度，不要空话。\n\n"
        "深度与广度检查（复杂题尽量覆盖）：\n"
        "- 原理链路（为什么有效）；\n"
        "- 落地动作（具体怎么做）；\n"
        "- 关键指标/阈值（如何判断好坏）；\n"
...
def _manual_text_prompt_body(language: str, language_lower: str) -> str:
    return (
        ...
        + _FIRST_SENTENCE_CONSTRAINT
        + "\n表达目标：真人候选人口吻，像在现场回答面试官；允许更完整，但不要变成文档提纲或培训材料。\n"
        "先做题型判定，再按题型组织答案；题型判定只在内部使用，不要把题型模板标题原样输出。\n\n"
        "优先命中以下三类面试高频：\n"
        "1. 场景题/系统设计/故障排查题；\n"
