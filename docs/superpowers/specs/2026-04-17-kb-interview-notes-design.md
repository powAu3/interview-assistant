...
- 在手动问 / 笔试 / 实时辅助 ASR 三种模式下，LLM 生成答案前**按 `kb_trigger_modes` 配置**自动检索 top-k 片段，作为「事实参考」拼进 prompt；MVP 三种模式默认全开，ASR 走更短的 `kb_asr_deadline_ms=80ms` + 碎句 early return 保护主流程。
...
- **MVP 不支持 `.doc`（老 Word 二进制）**：前端上传直接拒收（扩展名白名单不含），提示「请在 Word 里另存为 .docx」。不做 LibreOffice 转换、不做纯 Python 解析。
...
| `.doc` | ❌ 不支持 | — | 上传时 415 拒收；CLI 索引时 `kb_doc.status='failed'` + `error="不支持 .doc，请在 Word 里另存为 .docx"`；不再进入 loader dispatch | MVP 明确拒绝，避免引入 LibreOffice 依赖；未来也不计划做 |
| `.html` / `.epub` / `.pptx` / `.xlsx` | ❌ 非 MVP | — | 留 follow-up | 如有需求再按 loader pattern 加 |
...
注入点（由 `kb_trigger_modes` 控制启用；预算按模式取不同值）：

| 模式 | 启用 | 预算（来源） | 备注 |
|---|---|---|---|
| `PROMPT_MODE_ASR_REALTIME` | ✅ 默认启用 | `kb_asr_deadline_ms`，默认 **80ms** | 首 token 路径最紧，短预算 + 超时降级，主流程完全不受影响 |
| `PROMPT_MODE_MANUAL_TEXT` | ✅ 默认启用 | `kb_deadline_ms`，默认 **150ms** | 用户按钮触发，可容忍稍长 |
| `PROMPT_MODE_WRITTEN_EXAM` | ✅ 默认启用 | `kb_deadline_ms`，默认 **150ms** | 同上 |
| `PROMPT_MODE_SERVER_SCREEN` | ❌ 不启用 | — | 屏幕识题已有图像上下文，KB 召回噪音大 |

KB 段位置放在 `resume_section` 之后、`conversation_history` 之前，并显式声明「仅作事实参考」。

**ASR 模式的额外保护**：除了 80ms 预算外，`retrieve` 内部会把 query 归一化后再判空（短于 6 字符 或 全是常见噪声词 → 直接 early return 空列表，零 SQLite 开销）。

### Deadline-driven integration (保护主流程)

```python
async def build_prompt_for_asr(...):
    kb_hits = []
    if cfg.kb_enabled and "asr_realtime" in cfg.kb_trigger_modes:
        try:
            kb_hits = await run_in_threadpool(
                kb_retriever.retrieve,
                query=current_transcription,
                k=cfg.kb_top_k,
                deadline_ms=cfg.kb_asr_deadline_ms,  # ASR 专用，默认 80
            )
...
**硬约束**：
- **`retriever.retrieve` 必须被 `run_in_threadpool` 包起来调用**，永不同步阻塞事件循环。后端调用方（prompts 构建、`/api/kb/search` 端点）统一遵守。
- `retrieve` 内部必须**自己再实现一层** deadline：SQL 执行 + wall clock 计时 + `conn.interrupt()`（从 retriever 自己启动的 watchdog 线程调用），不依赖外层 `wait_for` 兜底。
- 任何异常都被吞掉并 `_log.warning`，对主流程透明。
- 主流程不持有 SQLite 连接：每次 `retrieve` 开短连接、用完关。避免 WAL 锁竞争。
- ASR 模式额外：query 短于 6 字符 / 只有噪声词 → 第一行 early return，不开 SQLite 连接。
...
| `kb_deadline_ms` | `150` | 手动 / 笔试模式主流程预算（毫秒） |
| `kb_asr_deadline_ms` | `80` | ASR 实时模式主流程预算（毫秒，首 token 路径更紧） |
| `kb_min_score` | `1.0` | BM25 过滤阈值（越低越宽） |
| `kb_chunk_max_chars` | `800` | 单片段最大字符 |
| `kb_prompt_excerpt_chars` | `300` | prompt 里单片段截断到多少字符 |
| `kb_trigger_modes` | `["asr_realtime","manual_text","written_exam"]` | 哪些 prompt 模式启用 KB（三种默认全开，ASR 靠短 deadline 保护） |
| `kb_file_extensions` | `[".md",".txt",".log",".docx",".pdf"]` | 允许索引与上传的扩展名白名单（`.doc` 不在内，上传 415 拒收） |
| `kb_ocr_enabled` | `false` | PDF L2 OCR（rapidocr-onnxruntime） |
| `kb_vision_caption_enabled` | `false` | PDF L3 Vision caption（复用项目已配置的 vision 模型，与 vision_verify 同一套） |
| `kb_max_upload_bytes` | `20 * 1024 * 1024` | 前端上传单文件大小上限（20MB） |
| `kb_recent_hits_capacity` | `50` | 最近命中日志的环形缓冲大小 |
| `kb_asr_min_query_chars` | `6` | ASR 模式下，query 短于此值直接 early return（避免碎句查 KB） |
...
| 超时（>`kb_deadline_ms` / `kb_asr_deadline_ms`，按触发模式取值） | 强制中断 + 返回空 |
...
| 上传 / CLI 放入 `.doc` 文件 | 上传 415；CLI 索引 `kb_doc.status='failed'` + 文案「不支持 .doc，请另存为 .docx」，前端红色高亮 |
...
- `test_indexer.py`：全量 reindex、mtime + size 增量、扩展名白名单过滤（尤其 `.doc` 在 CLI 入口被识别并 `status='failed'`，提示文案正确）。
...
- **MVP 范围**：
  ...
  - OCR / Vision 全部 opt-in，后端装了依赖就能开，前端 UI 提示「未安装 xxx 时请 `pip install yyy`」；`.doc` 不支持（前端拒收 + CLI failed）。
...
10 项 open question 全部定稿，已回填到对应章节。保留在此留底：

| # | 决策 | 落地位置 |
|---|---|---|
| D1 | **`data/kb/**` 不进 git**（本地私有）；`kb.sqlite` / `kb_cache/` 同样 .gitignore | `.gitignore` 新增三条 |
| D2 | **ASR 默认启用 KB**，但采用更短的 `kb_asr_deadline_ms=80`（vs 其他模式 150）+ query 短于 6 字符 early return，确保不影响首 token | Config 新增 `kb_asr_deadline_ms` / `kb_asr_min_query_chars`；Prompt injection 章节双预算表；Deadline-driven integration 硬约束 |
| D3 | **上传大小上限 20MB** | `kb_max_upload_bytes=20MB` |
| D4 | **`.doc` 完全不支持**：前端拒收、CLI 索引标 failed、不做 LibreOffice 转换、不做纯 Python 解析 | Non-goals / Document format support 表 / Error handling |
| D5 | **OCR = `rapidocr-onnxruntime`**（纯 pip、中文友好、无系统依赖） | PDF image handling 章节 / 依赖声明 |
| D6 | **Vision caption 复用项目已配置的 vision 模型**（与 `vision_verify` 同一套，零新增配置） | PDF image handling 章节 / Rollout |
| D7 | **前端入口 = 顶栏图标按钮 → `KnowledgeDrawer`** | Frontend UX 章节 |
| D8 | **chunker 超长原子单元：截断 + `[截断]` 标记** | Chunking rules |
| D9 | **`retriever` 强制走 `run_in_threadpool`**，永不同步阻塞事件循环 | Deadline-driven integration 硬约束 |
| D10 | **WS `kb_hits` `scope='global'`**，所有模式都收 | Frontend UX / Payload 示例 |

## Open questions (implementation 阶段再讨论，非阻塞)

- 中文分词：`unicode61` 够不够用 / 什么时候升级 `jieba`？（建议跑一周看召回率再决定）
- `BoundedTaskWorker` 复用 or 为 KB 新建一个（上传 + 索引可能同时发生，并发度要隔离）？
- 命中日志环形缓冲放进程内存即可，还是要 sqlite 表持久化（重启保留）？
- 前端检索测试框的结果是否与"最近命中日志"共用同一视觉组件？
