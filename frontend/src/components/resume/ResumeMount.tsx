...
export function ResumeMountInline({
  ...
  return (
    ...
    >
      ...
      />
      {uploading ? (
        <div className="flex items-center gap-1 pl-2 pr-1 py-1.5 text-xs text-text-muted rounded-l-lg">
          <Loader2 className="w-3 h-3 animate-spin flex-shrink-0" />
          <span className="hidden sm:inline">解析中…</span>
        </div>
      ) : hasResume ? (
        <div className="flex items-center gap-1 pl-2 pr-1 py-1.5 text-xs rounded-l-lg min-w-0 max-w-[220px]">
          <FileText className="w-3 h-3 text-accent-green flex-shrink-0" />
          <span
